...
class ClosedTrade(OpenedTrade):
	...
	def __str__(self):
		return "%s\n%s\n%s\n%s\n%s" % (self.Type, self.Date, self.Shares, self.Entry, self.Exit)
...
class Position:
	...
	def Show(self):
		...
		print("Entry:  %s"   % self.EntryPrice)
		print("Shares: %s"   % self.Shares)
		print("Exit:   %s"   % self.ExitPrice)
		print("Stop:   %s\n" % self.StopLoss)
...
# Important for cryptos!
# Need to add liquidation point
